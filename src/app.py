...
def thumbLocal(localFile, keyFile, size):
    ...
    s3_client.upload_file(local_resized, 'bucket-store-pillowv1', re.sub(regex, '_'+size+'_', keyFile, 0, re.MULTILINE))
...
def lambda_handler(event, context):
    for record in event['Records']:
        ...
        s3_client.upload_file(localFile, 'bucket-store-pillowv1', key)
